...
router.get('/', async (req, res) => {
	...
	async function TANJIRO_V1_QR_CODE() {
		...
		try {
			let Qr_Code_By_nerdk_Tech = nerdk_Tech({
				auth: state,
			...
			Qr_Code_By_nerdk_Techr.ev.on("connection.update", async (s) => {
				const {
					connection,
					lastDisconnect,
					qr
				...
				if (connection == "open") {
				   ...
				   let session = await Qr_Code_By_nerdk_Tech.sendMessage(Qr_Code_By_Venocyber_Tech.user.id, { text: '' + b64data });
	
				   let TANJIRO_V1_TEXT = `
*_Qr Code By nerdk Tech_*
*_Made With 🤍_*
